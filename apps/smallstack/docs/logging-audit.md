...
**Why this matters:** When a user reports "I got an error," you can ask for the request ID from their browser's network tab (or your frontend can log it automatically). That one string then finds both the `RequestLog` row (status code, response time, user) and every log line the request produced — including the traceback, and including Django's own auto-logged 4xx/5xx access-log line (`Unauthorized: /api/...`, `Not Found: /api/...`) — the most common thing a frontend integrator hits first (a bad/expired token, a mistyped URL). That line is logged by Django's own machinery *after* the request's ID would normally have been unbound, but `RequestContextFilter` falls back to reading it off the request object the log call still carries, so it isn't lost.
...
**Reading it from a cross-origin frontend:** "your frontend can log it automatically" only works if `response.headers.get("X-Request-ID")` actually returns the value — which requires `CORS_EXPOSE_HEADERS = ["X-Request-ID"]` (already set in `config/settings/base.py`) for any frontend running on a different origin than the API. See the CORS section of `docs/skills/api.md` / `docs/skills/frontend-integration.md`.
...
**Background tasks and scheduled jobs already do this for you.** Every task run through `apps.tasks`/`manage.py db_worker` gets a `trace_task_<id>` bound automatically around its execution, and `apps.scheduler`'s tick binds a `trace_schedjob_<job pk>_<tick>` around each job's own tick-time processing — no `bind_trace_id()` call needed in your task or job code. Reach for the pattern above yourself for work outside those two systems: a webhook delivery chain, a multi-step pipeline you're driving directly.
...
Keys can't collide with `LogRecord` built-ins (`name`, `module`, `message`, `args`, `levelname`, …) — Python raises `KeyError` at the call site if they do. Values that aren't JSON-serializable are rendered with `repr()` rather than dropped — truncated to 2000 characters if the rendered form is huge, so one hostile object can't blow up a log line or a database row.
...
- **Every record links to its request or trace.** Expanding a row shows its `request_id` and/or `trace_id`; clicking either filters to every line that request/trace produced, in order. The filter bar also has a `trace_id` text box, so you can paste one in directly — the same affordance `request_id` has via `/smallstack/activity/requests/`'s `logs` link on each request.
...
Windows live in the database, not in one process's memory, so every worker and every container picks up the change within about five seconds — including a process that starts *after* the window opened and hasn't logged anything at all. The writer/poller thread that does the polling is started eagerly from `TelemetryConfig.ready()` (`apps/telemetry/apps.py`), before the process has done any work of its own — not lazily on that process's first log call, and not gated behind whether that first call happens to clear the baseline level. One caveat: under gunicorn's `preload_app = True` (not the shipped default — `smallstack/gunicorn.conf` doesn't set it, so workers fork before the app loads and `ready()` runs in every worker), a thread started in the master by `ready()` doesn't survive `fork()`; each forked worker falls back to starting its poller lazily on its own first log line instead (still any level, not gated behind the level check), which works the same way but with a small delay until that worker's first line. Each row records who opened it and why, which leaves a small audit trail of when the system was running verbose.
