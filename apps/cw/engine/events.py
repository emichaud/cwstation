...
these events, so the manager, the log bridge, and the live view never need to
know which engine produced a given character. This contract is what makes the
audio layer a general seam rather than a CW-specific hack.
"""
...
@dataclass
class DecodeResult:
    """Everything a decode pass produced — text plus the telemetry the live
