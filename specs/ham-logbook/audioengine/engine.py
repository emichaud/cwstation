...
* `AudioDemodulator` — consumes audio blocks and decodes them itself. The CW
  decoder is the first; an ML/LSTM decoder later is a drop-in sibling.
...
resulting `CharEvent`s to subscribers (the logbook bridge, the live view).
Adding a new mode later = writing one class + registering it. That is the whole
point of building this up front.
"""
...
class NetworkTapEngine(Engine):
    """An engine fed by an external process (fldigi/WSJT-X). Stubbed here so the
...
class AudioEngineManager:
    ...
    def pump(self, samples: np.ndarray) -> None:
